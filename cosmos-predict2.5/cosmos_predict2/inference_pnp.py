...
class Inference:
    ...
    def _generate_sample(self, sample: InferenceArguments, output_dir: Path) -> str | None:
        ...
        if sample.enable_autoregressive:
            raise NotImplementedError("Autoregressive mode is not implemented in video2world PnP yet.")
            ...
            video = self.pipe.generate_vid2world_pnp(
                prompt=sample.prompt,
                input_path=path_to_str(sample.input_path),
                guidance=sample.guidance,
                ...
                stochastic_plan=sample.stochastic_plan,
                ths_uncertainty=sample.ths_uncertainty,
                p_norm=sample.p_norm,
                certain_percentage=sample.certain_percentage,
            )
