...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __call__(
        ...
        max_sequence_length: int = 512,
        #### hyperpamaters for pnp ####
        stochastic_plan: List[Union[Tuple[int, int, int], Dict[str, int]]] = [(2, 5, 3), (6, 14, 1)],
        ths_uncertainty: float = 0.0,
        p_norm: int = 1,
        certain_percentage: float = 0.999, # if the certain region is more than this percentage, set certain_flag to True
    ):
        r"""
        ...
        Args:
            ...
            stochastic_plan (`List[Union[Tuple[int, int, int], Dict[str, int]]]`, defaults to `[(2, 5, 3), (6, 14, 1)]`):
                Stochastic plan for predict-and-perturb. List of (start, end, steps) tuples or list of dicts with 'start', 'end', 'steps' keys.
            ths_uncertainty (`float`, defaults to `0.0`):
                Threshold for uncertainty to determine certain and uncertain regions.
                Lower values increases refinement strength, however, it may also introduce artifacts.
                (I2V does not require high threshold since the input image prevents over-saturation issue.)
            p_norm (`int`, defaults to `1`):
                The norm degree for calculating uncertainty. We used L1 norm.
            certain_percentage (`float`, defaults to `0.999`):
        ...
        def _build_stochastic_step_map(
            plan: Optional[List[Union[Tuple[int, int, int], Dict[str, int]]]]
        ) -> Dict[int, int]:
            """
            ...
            def _to_int(name: str, value: Any) -> int:
                try:
                    ...
                    raise ValueError(f"Unable to cast '{name}' to int for stochastic_plan entry: {value}") from exc
            ...
            for entry in plan:
                ...
                start_i = _to_int("start", start)
                end_i = _to_int("end", end)
                steps_i = _to_int("num_anneal_steps", steps)
        ...
        # Hyperparameter setup for predict-and-perturb
        stochastic_step_map = _build_stochastic_step_map(stochastic_plan)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                buffer = [None] # buffer for the next latent (certain_mask, pred_original_sample, latents_next)
                certain_mask = None
                certain_flag = False
